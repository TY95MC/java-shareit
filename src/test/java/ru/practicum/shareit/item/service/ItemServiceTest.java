...
class ItemServiceTest {
    ...
    void shouldFailAddNewItem() {
        ...
        e = assertThrows(EntityValidationException.class,
                () -> itemService.addNewItem(owner.getId(), inputItemDto));
        assertThat(e.getMessage(), equalTo("Проверьте корректность данных новой вещи!"));
    }
    ...
    void shouldFindByTextSuccessfully() {
        ...
        text = "";
        tmp = itemService.findByText(owner.getId(), text, 0, 10);
        assertThat(tmp.isEmpty(), equalTo(true));
    }
